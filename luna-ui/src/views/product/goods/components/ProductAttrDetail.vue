          ...
          <div v-for="(productAttr,idx) in selectProductAttr">
            ...
            <el-checkbox-group v-if="productAttr.handAddStatus===0" v-model="selectProductAttr[idx].values">
              <el-checkbox v-for="item in getInputListArr(productAttr.inputList)" :label="item" :key="item"
                           class="littleMarginLeft" @change="handleRefreshProductSkuList(false)"></el-checkbox>
            </el-checkbox-group>
            <div v-else>
              <el-checkbox-group v-model="selectProductAttr[idx].values">
                <div v-for="(item,index) in selectProductAttr[idx].options" style="display: inline-block"
                     class="littleMarginLeft">
                  <el-checkbox :label="item" :key="item" @change="handleRefreshProductSkuList(false)"></el-checkbox>
      ...
      <el-form-item label="属性图片：" v-if="hasAttrPic">
        <el-card shadow="never" class="cardBg">
          <div v-for="(item,index) in selectProductAttrPics">
            <span>{{ item.name }}:</span>
            <image-upload style="width: 300px;display: inline-block;margin-left: 10px" v-model="item.pic"/>
          </div>
        </el-card>
      </el-form-item>
      ...
      <el-form-item label="规格参数：">
        <el-tabs v-model="activeHtmlName" type="card">
          <el-tab-pane label="电脑端详情" name="pc">
            <editor v-model="value.detailHtml" :min-height="300"/>
          </el-tab-pane>
          <el-tab-pane label="移动端详情" name="mobile">
            <editor v-model="value.detailMobileHtml" :min-height="300"/>
          </el-tab-pane>
        </el-tabs>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleRefreshProductSkuList(val) {
      if (!val) {
        this.refreshProductAttrPics();
    ...
    handleSyncProductSkuPrice() {
      this.$confirm('将同步第一个sku的价格到所有sku,是否继续', '提示', {
        ...
        if (this.value.skuStockList !== null && this.value.skuStockList.length > 0) {
          ...
          let tempJson = JSON.stringify(tempSkuList);
          this.value.skuStockList = JSON.parse(tempJson);
        }
      });
    },
    ...
    handleRedoProductSku(index, row) {
      let list = this.value.skuStockList;
      console.log(list);
      let data = list[index];
      for (let i = 0; i < list.length; i++) {
        list[i].stock = data.stock;
